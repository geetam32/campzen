...
import { Calendar, Clock, Book, User, MapPin } from 'lucide-react-native';
                ...
                <ScrollView style={styles.list} contentContainerStyle={styles.listContent}>
                    {daySlots.map(slot => (
                        <View key={slot.period} style={styles.periodRow}>
                            ...
                            {slot.data ? (
                                <View style={[styles.slotCard, slot.data.is_lab && styles.labSlot, slot.data.subject === 'Study Hour' && styles.studySlot]}>
                                    ...
                                    <View style={styles.slotFooter}>
                                        <View style={styles.metaItem}>
                                            ...
                                            <Text style={styles.metaText}>{slot.data.teacher_name || 'Teacher'}</Text>
                                        </View>
                                        <View style={styles.metaItem}>
                                ...
                                <View style={styles.emptySlot}>
                                    <Text style={styles.emptyText}>Free Period</Text>
                                </View>
                            )}
                        </View>
                    ))}
...
const styles = StyleSheet.create({
    ...
    slotCard: { flex: 1, padding: 16, borderRadius: 16, backgroundColor: '#fff', borderWidth: 1, borderColor: '#e2e8f0' },
    labSlot: { backgroundColor: '#f5f3ff', borderColor: '#c7d2fe' },
    studySlot: { backgroundColor: '#f0fdf4', borderColor: '#bcf0da' },
    ...
    labBadge: { backgroundColor: '#6366f1', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4 },
    labText: { color: '#fff', fontSize: 10, fontWeight: 'bold' },
    slotFooter: { flexDirection: 'row', gap: 16 },
    metaItem: { flexDirection: 'row', alignItems: 'center', gap: 6 },
    metaText: { fontSize: 12, color: '#64748b' },
    emptySlot: { flex: 1, backgroundColor: 'rgba(241, 245, 249, 0.4)', borderRadius: 16, justifyContent: 'center', paddingHorizontal: 16, borderStyle: 'dashed', borderWidth: 1, borderColor: '#e2e8f0' },
